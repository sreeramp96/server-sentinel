                                                ...
                                                @forelse($websites as $website)
                                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                                        <td class="px-6 py-4 whitespace-nowrap">
                                                            ...
                                                            <div class="text-xs text-gray-400 mt-0.5">Added {{ $website->created_at->diffForHumans() }}
                                                            </div>
                                                        ...
                                                        <td class="px-6 py-4 whitespace-nowrap">
                                                            <div x-data="{ open: false }" class="relative">
                                                                ...
                                                                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                                                    ...
                                                                    class="absolute right-0 z-20 mt-1 w-52 rounded-lg shadow-lg bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 py-1"
                                                                    style="display: none;">

                                                                    {{-- Toggle public status page --}}
                                                                    <form method="POST" action="{{ route('websites.togglePublic', $website) }}">
                                                                        @csrf
                                                                        @method('PATCH')
                                                                        <button type="submit"
            ...
            @if($websites->isNotEmpty())
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 dark:border-gray-700">
                    <div class="p-6">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Outages detected across all your monitored sites.</p>

                        @php
    $allIncidents = $websites->flatMap(fn($w) => $w->loadMissing('incidents')->incidents)->sortByDesc('started_at')->take(20);
                        @endphp

                        @if($allIncidents->isEmpty())
                            <div class="flex items-center gap-3 px-4 py-5 border border-dashed border-gray-200 dark:border-gray-700 rounded-lg text-center justify-center">
                                <svg class="h-5 w-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd"/>
                                </svg>
                                <p class="text-sm text-gray-500 dark:text-gray-400">No incidents recorded — all sites have been healthy.</p>
                            </div>
                        @else
                            <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        @foreach($allIncidents as $incident)
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                                                    ...
                                                    <div class="text-xs text-gray-400">{{ $incident->started_at->diffForHumans() }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                                                    {{ $incident->resolved_at ? $incident->resolved_at->format('d M, H:i') : '—' }}
                                                </td>
